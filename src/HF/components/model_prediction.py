...
class ModelPrediction:
    ...
    def load_model(self):
        try:
            # Try to load the model from the configured path
            logging.info(f"Loading model from {self.model_prediction_config.model_file_path}")
            if os.path.exists(self.model_prediction_config.model_file_path):
                model = joblib.load(self.model_prediction_config.model_file_path)
                return model

            # If the model is not found, try to load from best_xgboost_model.pkl
            alternate_path = "best_xgboost_model.pkl"
            if os.path.exists(alternate_path):
                logging.info(f"Model not found at {self.model_prediction_config.model_file_path}. Loading from {alternate_path}")
                model = joblib.load(alternate_path)

                # Copy the model to the expected location for future use
                os.makedirs(os.path.dirname(self.model_prediction_config.model_file_path), exist_ok=True)
                joblib.dump(model, self.model_prediction_config.model_file_path)
                logging.info(f"Copied model to {self.model_prediction_config.model_file_path}")

                return model

            # If no model is found, raise an exception
            raise FileNotFoundError(f"Model not found at {self.model_prediction_config.model_file_path} or {alternate_path}")
